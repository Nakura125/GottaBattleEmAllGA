...
public class Prediction {
    ...
    public static double predict(Team team1, Team team2){
        List<Team> result = ResultGA.getResult();
        Integer max1=0;
    ...
    private static Integer distance(Team team1, Team team2){
        ...
        for(int i=0; i< dim; i++){
            ...
            if(Objects.equals(team1.getPokemon(i).getNome(), team2.getPokemon(i).getNome()))
                sum+=1;

            //se i pokemon hanno lo stesso tipo
            if(Objects.equals(team1.getPokemon(i).getTipo1(), team2.getPokemon(i).getTipo1()) ||
                    Objects.equals(team1.getPokemon(i).getTipo1(), team2.getPokemon(i).getTipo2()))
                sum+=1;

            //se i pokemon hanno lo stesso tipo
            if(Objects.equals(team1.getPokemon(i).getTipo2(), team2.getPokemon(i).getTipo1()) ||
                    Objects.equals(team1.getPokemon(i).getTipo2(), team2.getPokemon(i).getTipo2()))
                sum+=1;

            //se i pokemon hanno lo stesso totale
            if (team1.getPokemon(i).getTotale()== team2.getPokemon(i).getTotale())
                sum+=1;


            //se i pokemon hanno le mosse uguali
            for (int j=0; j<4; j++)
                if(team1.getPokemon(i).getMosse().get(j)!=null)
                    for (int k=0; k<4; k++)
                        if(team2.getPokemon(i).getMosse().get(k)!=null)
            ...
            for (int j=0; j<4; j++)
                if(team1.getPokemon(i).getMosse().get(j)!=null)
                    for (int k=0; k<4; k++)
                        if(team2.getPokemon(i).getMosse().get(k)!=null)
                            if(Objects.equals(team1.getPokemon(i).getMosse().get(j).getTipo(), team2.getPokemon(i).getMosse().get(k).getTipo()))
                                sum+=1;
        }
